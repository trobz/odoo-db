    ...
    with _writer() as w:
        ...
        if w.fmt == "json":
            ...
            if db.has_cron_failure_data(rows_data):
                failing = sum(1 for r in rows_data if r.get("failure_count") and r.get("active", True))
                lines += [
                    "# HELP odoo_db_crons_failing Crons currently accumulating consecutive failures (Odoo 18+)",
                    "# TYPE odoo_db_crons_failing gauge",
                    f'odoo_db_crons_failing{{db="{db_name}"}} {failing}',
                ]
            w.prometheus(lines)
        else:
            show_failure_columns = db.has_tracked_cron_failures(rows_data)
            header = ["name", "interval", "nextcall"]
            if include_code:
                header.append("code")
            if include_inactive:
                header.append("active")
            if show_failure_columns:
