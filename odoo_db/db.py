...
def has_running_cron_progress(rows: list[dict]) -> bool:
    """Any `--running` row carries real progress (`done` or `remaining`
    truthy) -- gates the running-crons text table's progress columns, same
    reasoning as `has_tracked_cron_failures`: the LATERAL join attaches the
    keys to every row on Odoo 18+, but `_add_progress()` writes a `done=0,
    remaining=0` row on every attempt before the callback runs, so an
    `is not None` check would fire on that placeholder row too. Only a
    nonzero `done`/`remaining` means a cron in flight has actually reported
    progress; three empty/zero columns are worse than none.
    """
    return any(r.get("done") or r.get("remaining") for r in rows)
