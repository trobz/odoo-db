...
| `crons <db>` | List active scheduled actions (`--all` also includes inactive ones). `--running` shows crons currently held by an Odoo worker (RowShareLock on `ir_cron`) — transient debug data, not bundled into `prepare-audit`. `--include-code` adds the python source of each `state='code'` cron (ignored with `--running`, which already always shows it). On Odoo 18+, also surfaces `failure_count`/`first_failure_date` (both modes) and `--running`'s live `done`/`remaining`/`timed_out_counter` progress — omitted entirely on older databases; JSON always includes the keys, the text table only shows the corresponding columns when a row actually has a nonzero value. `--output-format prometheus` adds a `odoo_db_crons_failing` gauge (active crons with `failure_count > 0`, Odoo 18+ only, unaffected by `--all`) alongside the existing `odoo_db_crons_active` |
...
| `check-passwords <db>` | Detect active users whose password is trivially guessable, by testing each user's stored pbkdf2 hash against single characters, a small common-password list (`admin`, `odoo`, …) and the login itself — purely local, no login attempts. Per candidate cost is one full pbkdf2 (600k rounds in core), so the scan is parallelized over threads (`hashlib.pbkdf2_hmac` releases the GIL); expect seconds per user. Findings carry a `reason` (`single_char`, `common`, `same_as_login`, plus `plaintext`/`malformed` for stored values that are findings in themselves — any unparsable hash is `malformed`, never silently clean); each user is keyed by `user_id`, `login` (PII) shown only with `--include-sensitive-information`. `--output-format prometheus` exposes `odoo_db_weak_passwords{reason=...}`, one series per reason including zeros (sum over reasons for the total) |
